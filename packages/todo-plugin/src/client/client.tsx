...
export function todosLoader(config: TodosClientConfig) {
  return async () => {
...
export function createTodosMeta(config: TodosClientConfig, path: string) {
  return () => {
...
export function createAddTodoMeta(config: TodosClientConfig, path: string) {
  return () => {
    const { siteBaseURL, siteBasePath } = config;
    const fullUrl = `${siteBaseURL}${siteBasePath}${path}`;
  ...
  defineClientPlugin({
    ...
    routes: () => ({
      todosList: createRoute("/todos", () => {
        // Lazy load to avoid "use client" propagation to plugin definition
      ...
      addTodo: createRoute("/todos/add", () => {
        // Lazy load to avoid "use client" propagation to plugin definition
        const AddTodoPageComponent = lazy(() =>
          import("./pages/add-todo").then((m) => ({
            default: m.AddTodoPageComponent,
          }))
        );
        
        return {
          PageComponent: AddTodoPageComponent,
